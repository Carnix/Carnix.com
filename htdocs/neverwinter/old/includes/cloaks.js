...
if(typeof Carnix === "undefined"){
	var Carnix = {};
	if(typeof Carnix.Cloaks === "undefined"){
		Carnix.Cloaks = {};
	}
}
...
Carnix.Cloaks.GuildDataClass = function(config){
	/*	@name			GuildDataClass
	 *	@this			Carnix.Cloaks.GuildDataClass
	 */
	this.Class={
		/*	@name			Static configuration
		 *	@this			Carnix.Cloaks.GuildDataClass.Class
		 */
		static: {//optional static configuration variables.
			"statickey1":"staticvalue1",
			"statickey2":"staticvalue2"
		},

		/*	@name			Dynamic configuration
		 *	@this			Carnix.Cloaks.GuildDataClass.Class.Class
		 */
		config:{//optional configuration variables.
			"key1":"value1",
			"key2":"value2"
		},
		...
		constructor: function(config){
			this.config = $.extend(true,this.config,config);//inserts (destructively!) the instance config variables into the config collection
			this.config = $.extend(true,this.config,this.static);//inserts (destructively!) the static variables into the config collection
		...
		ExecuteSearch: function(value,type){
	...
	return this.Class.constructor(config); //executes the constructor method.
};

/*	@description	Instantiates an instance of Carnix.Cloaks.GuildDataClass.  Typically place on pages where this class needs to be used.
 *	@this			window

	var MyClassInstance = new MyRootObj.MyClassNamespace.MyClassName({
		"instanceKeyOne":"instance_value_1",
		"instanceKey2":"instance_val_two",
	});
 */
